-- ══════════════════════════════════════════════════════════════════════
-- GASTROMUNDI — as 33 migrações pendentes, na ordem certa.
--
-- Gerado a partir de supabase/migrations/, de 20260918_delivery_nao_publica_insumo_nem_preco_zero.sql
-- até 20261011_vitrine_mostra_grupos_do_produto.sql.
--
-- COMO USAR
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [01/33] 20260918_delivery_nao_publica_insumo_nem_preco_zero.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [02/33] 20260918_grupos_escolha.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [03/33] 20260919_baixa_estoque_cria_linha.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [04/33] 20260919_delivery_entregadores.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [05/33] 20260919_pautas.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [06/33] 20260920_delivery_pagamento_entregador.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [07/33] 20260920_rls_habilitada_tabelas_base.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [08/33] 20260921_delivery_rate_limit_sem_telefone.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [09/33] 20260921_indices_tenant_id.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [10/33] 20260922_fiscal_config_leitura_por_papel.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [11/33] 20260922_gravar_itens_comanda.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [12/33] 20260923_combo_produtos_isolamento_tenant.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [13/33] 20260923_dinheiro_precisao.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [14/33] 20260924_fechamentos_colunas.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [15/33] 20260924_search_path_security_definer.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [16/33] 20260925_leads_apex.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [17/33] 20260925_ponte_download_bucket.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [18/33] 20260926_solicitacoes_conta.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [19/33] 20260927_grupo_escolha_itens_ativo.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [20/33] 20260928_delivery_retirada_no_local.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [21/33] 20260929_delivery_sem_cep_e_meus_pedidos.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [22/33] 20260930_delivery_telefone_obrigatorio.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [23/33] 20261001_delivery_via_sai_sozinha.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [24/33] 20261002_venda_do_delivery.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [25/33] 20261003_delivery_whatsapp_no_aceite.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [26/33] 20261004_escolhas_sem_limite.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [27/33] 20261005_desabilitar_produto_espelho.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [28/33] 20261006_cliente_do_delivery.sql'; END $aplicando$;
...
-- ══════════════════════════════════════════════════════════════════════
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [29/33] 20261007_delivery_sincroniza_produto_novo.sql'; END $aplicando$;
...
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [30/33] 20261008_regra_preco_grupo.sql'; END $aplicando$;
...
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [31/33] 20261009_integridade_do_historico.sql'; END $aplicando$;
...
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [32/33] 20261010_feedbacks.sql'; END $aplicando$;
...
DO $aplicando$ BEGIN RAISE NOTICE '▶ APLICANDO [33/33] 20261011_vitrine_mostra_grupos_do_produto.sql'; END $aplicando$;
