...
-- ── fechamentos ───────────────────────────────────────────────
-- `data` é o registro do fechamento, gravado pelo app. As colunas abaixo
-- são projeção dele, calculadas pelo próprio banco (20260924): existem para
-- o fechamento ser consultável — "quais caixas fecharam com falta este mês",
-- "diferença por operador" — sem que ninguém tenha de gravar o mesmo número
-- duas vezes. `total_esperado` e `diferenca` repetem a regra de
-- esperadoEmCaixa()/diferencaCaixa() em src/lib/caixa.js, fallback dos
-- fechamentos antigos incluído; mudou lá, muda aqui.
CREATE TABLE public.fechamentos (
  ...
  -- 20260924 — colunas geradas (GENERATED ALWAYS AS ... STORED)
  usuario_nome         text          GENERATED ALWAYS AS (data ->> 'user') STORED,
  usuario_papel        text          GENERATED ALWAYS AS (data ->> 'role') STORED,
  fundo                numeric(12,2) GENERATED ALWAYS AS (
    CASE WHEN jsonb_typeof(data -> 'fundo') = 'number'
         THEN (data ->> 'fundo')::numeric END
  ) STORED,
  total_vendas         numeric(12,2) GENERATED ALWAYS AS (
    CASE WHEN jsonb_typeof(data -> 'totalVendas') = 'number'
         THEN (data ->> 'totalVendas')::numeric END
  ) STORED,
  total_esperado       numeric(12,2) GENERATED ALWAYS AS (
    COALESCE(
  ...
  total_conferido      numeric(12,2) GENERATED ALWAYS AS (
  ...
  -- jsonb porque a lista de meios de pagamento é configurável por tenant
  conferido_por_metodo jsonb         GENERATED ALWAYS AS (
    CASE WHEN jsonb_typeof(data -> 'conferidoPorMetodo') = 'object'
         THEN data -> 'conferidoPorMetodo' END
  ) STORED,
  observacao           text          GENERATED ALWAYS AS (data ->> 'observacao') STORED
);
CREATE INDEX fechamentos_tenant_diferenca_idx  -- 20260924
...
CREATE TABLE public.caixa_movimentos (
  ...
  valor            numeric(12,2)     NOT NULL,   -- CHECK: > 0 (o sinal está no tipo)
...
-- ── login_tentativas — 20260927_login_tentativas_servidor.sql ──
-- Rate limit do LOGIN (TD008): 5 falhas na janela de 15 min bloqueiam por 2 min.
-- A chave é `md5(usuario@slug.local)`, e não `auth.uid()`, porque o login
-- acontece antes de existir sessão. Guardar o digest, e não o e-mail, evita
-- deixar aqui uma lista legível de logins válidos. Linhas são descartáveis.
...
CREATE INDEX login_tentativas_janela_idx ON public.login_tentativas (janela_inicio);
...
CREATE TABLE public.delivery_entregadores (
  ...
  telefone          text,
  ativo             boolean NOT NULL DEFAULT true,
  valor_por_entrega numeric(12,2) NOT NULL DEFAULT 0,
  created_at        timestamptz NOT NULL DEFAULT now(),
  updated_at        timestamptz NOT NULL DEFAULT now()
);
...
-- login_tentativas_estado(p_chave text)                    → 20260927_login_tentativas_servidor.sql
--   (consulta o bloqueio do login antes do signInWithPassword; `anon`, porque é pré-sessão)
-- login_tentativas_falha(p_chave text)                     → 20260927_login_tentativas_servidor.sql
--   (só SOMA falha: 5 na janela de 15 min bloqueiam por 2 min; `anon`)
-- login_tentativas_sucesso()                               → 20260927_login_tentativas_servidor.sql
--   (zera o contador; SEM parâmetro de propósito, a chave sai do e-mail do JWT, e só
--    `authenticated` executa — se o `anon` pudesse zerar, o freio seria contornável — TD008)
...
-- login_tentativas     → sem policy: só as funções login_tentativas_* (SECURITY DEFINER) escrevem (TD008, 20260927)
