...
-- ── logs (LEGADO — sem policies em produção; inacessível via API;
--          substituído por operator_logs; candidato a remoção) ──
...
-- baixar_estoque(p_produto_id bigint, p_qtd numeric)       → 20260705_estoque_tabela.sql
...
-- logs                  → SEM POLICIES (RLS nega tudo — tabela legada)

-- ── Realtime ──────────────────────────────────────────────────
-- Habilitado no dashboard para: pending, estoque
