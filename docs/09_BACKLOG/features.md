...
| F015 | Layouts de impressão (nota fiscal, produção/cozinha, comprovante de pagamento) — templates configuráveis por estabelecimento (white-label, decisão 017) | 🟠 High | Backlog | — | Transversal — usa infra de impressão já existente (`views/impressao/`) |
| F016 | Enforcement de assinatura (billing counter) — vigência por ciclo pago (ex.: 30 dias); ao vencer sem renovação, **bloquear o sistema** com mensagem "Sua mensalidade está atrasada". Inclui período de carência/grace e aviso pré-vencimento. Complementa/estende F013 | 🔴 Critical | Backlog | [billing-flow.md](../05_FLUXOS/billing-flow.md) | Transversal — depende de F013 e gateway de pagamento |
| F017 | Integração TEF (pagamento por maquininha/terminal — cartão débito/crédito integrado ao PDV) | 🟠 High | Backlog | [PDV.md](../03_REGRAS_DE_NEGOCIO/PDV.md) | **Add-on pago transversal** (decisão 019): disponível em TODOS os planos por valor adicional, não é recurso de tier. Código **nativo** (hook no PDV) desde já; provedor TEF pago (SiTef/PayGo) é o que se adia por custo |
| F019 | Emissão de nota fiscal no pagamento (NFC-e/NF-e) — **add-on pago transversal** (decisão 019): opção em TODOS os planos por valor adicional. Hook nativo no fluxo de pagamento; layouts em F015 | 🟠 High | Backlog | [PDV.md](../03_REGRAS_DE_NEGOCIO/PDV.md) · [FINANCEIRO.md](../03_REGRAS_DE_NEGOCIO/FINANCEIRO.md) | Código nativo desde já; provedor fiscal pago é o que se adia por custo (ver Restrições de Custo) |
| F018 | Revisão completa da estrutura de CSS — layout totalmente intuitivo e **responsivo**, com separação CSS/JSX (decisão 018) e base para theming/white-label por tenant (decisão 017) | 🟠 High | Backlog | [02_DESIGN_SYSTEM/](../02_DESIGN_SYSTEM/) | Transversal — fixar padrão em ADR de theming/CSS antes de aplicar |
