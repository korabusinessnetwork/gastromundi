...
| TD019 | `npm test` em clone novo: 9 arquivos morrendo na importação por falta de `VITE_SUPABASE_*` (106 testes que nunca rodavam) e 4 asserções dependentes do fuso da máquina (passavam em UTC-3, quebravam em UTC) | 🧪 Testes | Alto (suíte não protegia o que dizia proteger) | Baixo | 🟠 High | Resolvido (2026-08-15) — `test.env` em `vitest.config.js` fixa `TZ` e injeta credenciais falsas; nenhuma linha de produção alterada; 207 arquivos / 3612 testes verdes |
| TD020 | Venda no caixa não descontava estoque de produto sem linha em `estoque` — e `addProduct` nunca criava essa linha, então valia para **todo** produto cadastrado pelo app. A tela de Estoque mostrava saldo "0" indistinguível de produto que acabou | 🔒 Confiabilidade | Alto (inventário simplesmente não existia para o catálogo criado no app) | Baixo | 🔴 Critical | Resolvido (2026-08-15) — migration `20260919` faz a RPC criar a linha antes de descontar (+ backfill), `addProduct` cria a linha junto com o produto, o PDV parou de vetar pelo mapa local e "RPC sem linha de volta" virou falha (`estoque_sem_linha`) em produto e subproduto |

### [TD018] Âmbar `#f59e0b` literal fora do Console

**Categoria:** Code Quality · **Impacto:** Médio · **Esforço:** Médio · **Prioridade:** 🟡 Medium · **Status:** Resolvido (2026-08-15)

**Descrição:** o token `--gm-warn` existe em `src/styles/tema.css:50` e está na lista de tokens que o tenant pode sobrescrever (`src/lib/tema.js:66`), mas mais de 30 lugares do aplicativo cravavam o hex `#f59e0b` (ou `rgba(245, 158, 11, …)`) direto no JSX e no CSS. Efeito prático: o estabelecimento white-label trocava a paleta inteira e a cor de atenção continuava a mesma (decisão 017). Dois comentários no código ainda afirmavam que a cor era "semântica fixa, não de marca", contradizendo o `DeliveryView.jsx`, que dizia o contrário.

**Regra adotada (para as próximas telas):** `#f59e0b` vira `--gm-warn` quando (a) significa **atenção/alerta** — banner de atraso, estoque baixo, comanda esquecida, status pendente —, ou (b) era o **único hex cru numa paleta que já estava tokenizada**. Paletas categóricas inteiramente literais ficam como estão: tokenizar 1 de 5 amostras quebra a coerência do conjunto, e mapear "gerente" ou "COFINS" num token de *aviso* é errado semanticamente.

**Solução aplicada:** `varColor(C.warn)` para cor sólida e `alfa(C.warn, "NN")` para blend (`alfa` monta o `color-mix` por cima do `var()`, então o blend segue o tema do tenant do mesmo jeito) em `PDVView/ComandaGrid.jsx`, `PDVView/MesaMapView.jsx`, `CozinhaView.jsx`, `EstoqueView.jsx`, `AdminView.jsx`, `ConfiguracoesView.jsx`, `JarvasPanel.jsx`, `FechamentoModal.jsx`, `AssinaturaBanner.jsx` e `constants/roles.js`; `var(--gm-warn)` / `color-mix` nos CSS `ImportarExportarTab.css`, `DemoClientes.css` e `comprovante.css` (este com fallback `var(--gm-warn, #f59e0b)`, no mesmo formato que o arquivo já usava para `--gm-accent`, porque a folha vai para a janela de impressão). Os comentários de `ConfiguracoesView.css` e `ComandaGrid.jsx` que negavam o token foram reescritos, e o de `colorAlfa.js` passou a descrever os dois casos de fallback reais.

**Resíduos declarados (não são esquecimento):**

- `AdminView.jsx` `COR_TIPO` (tipos de imposto) e `PDVView/index.jsx` `METODOS_COLOR` (métodos de pagamento): paletas categóricas 100% literais — regra (b) não se aplica.
- `src/utils/crypto.js` rampa de força de senha: escala contínua fraco→forte, não um estado de aviso.
- `MesaMapView.jsx` status `aberta` (`#eab308`): precisa ser distinguível de `reservada` (`--gm-warn`) lado a lado no mapa, e o design system ainda não tem um segundo tom de atenção. Fechar isso é decisão de design system, não de varredura.

### [TD020] Baixa no caixa que não virava baixa no estoque

**Categoria:** Confiabilidade · **Impacto:** Alto · **Esforço:** Baixo · **Prioridade:** 🔴 Critical · **Status:** Resolvido (2026-08-15)

**Descrição:** não existe coluna `controla_estoque` em `products` — o que define se um produto é controlado é a **existência da linha dele em `estoque`**. E `addProduct` nunca criou essa linha. Somado a isso, o PDV tinha `if (!(prodId in estoque)) continue;` ("produto sem entrada no mapa = sem controle de estoque") e a RPC `baixar_estoque` é um `UPDATE ... WHERE produto_id = ...` que, sem linha, afeta zero linhas **sem erro nenhum**. Resultado: todo produto cadastrado pelo app era vendido sem descontar nada, para sempre — enquanto a `EstoqueView` o listava com saldo "0", igual a um produto controlado que acabou. Não havia como distinguir na tela o que acabou do que nunca foi contado.

Dois agravantes independentes: a guarda dava ao **mapa em memória do aparelho** poder de veto sobre a baixa (carga de estoque que falha no bootstrap = venda inteira sem desconto, em silêncio), e `processarBaixaEstoque` tratava "RPC OK, zero linhas" como sucesso, devolvendo `anterior - qty` — o mesmo saldo inventado que originou o TD012, um andar abaixo de onde ele foi corrigido.

**Solução aplicada:** quatro mudanças que só funcionam juntas — migration `20260919_baixa_estoque_cria_linha.sql` (a RPC cria a linha com `tenant_atual_id()` antes de descontar, INSERT deliberadamente **antes** da checagem de idempotência para que o reenvio da fila offline também receba a linha de volta, mais backfill dos produtos existentes); `addProduct` criando a linha junto com o produto (a tela fica honesta antes da primeira venda, não a partir dela); remoção do veto do cliente no `useFinalizarPagamento`; e `estoque_sem_linha` como erro explícito em `processarBaixaEstoque` e em `baixarEstoqueSubproduto` (que ignorava o `data` da RPC por completo), caindo no alerta do Jarvas que o TD012 já criou. Tirar o veto sem essa última trocaria um pulo silencioso por uma mentira silenciosa. Spec em `specs/baixa-no-caixa-sempre-vira-baixa-no-estoque.md`.

**`estoque_subprodutos` fica de fora de propósito:** lá a linha nasce com o cadastro do subproduto, que tem `controla_estoque` de verdade. Linha faltando ali é inconsistência de dados — o app passa a acusar, não a criar por conta própria.

**Resíduo declarado (decisão de produto, não de código):** produto que ninguém quer controlar (couvert, taxa de serviço) passa a ter linha e a gerar alerta de venda sem estoque a cada venda, e hoje não há como desligar o controle por produto — a saída é dar entrada do saldo. Um `controla_estoque` em `products` (com a baixa pulando quem estiver desmarcado) resolve, e é o próximo passo natural se o barulho incomodar. Continua pendente, do TD012, avisar o **operador** na tela do PDV quando a baixa não é aplicada.

### [TD019] Suíte de testes refém do relógio e do `.env.local` da máquina

**Categoria:** Testes · **Impacto:** Alto · **Esforço:** Baixo · **Prioridade:** 🟠 High · **Status:** Resolvido (2026-08-15)

**Descrição:** em clone novo, `npm test` dava `13 arquivos falhando | 4 testes falhando`, e nenhuma das falhas era bug de produção:

1. **9 arquivos morriam na importação** com `VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY ausentes` (`src/lib/supabase.js:13`). Qualquer teste que alcançasse `utils/hooks.js` puxava o client real e explodia antes do primeiro `it()` — mesmo com o Supabase dublado no teste. Sem `.env.local` (que não é versionado), essas 9 suítes simplesmente não rodavam: 106 testes invisíveis.
2. **4 asserções dependiam do fuso da máquina** (`assinatura.test.js:276` e `:324`, `console.test.js:970`, `AnalyticsDashboard.test.jsx:201`). O código de produção está certo: vencimento de assinatura, "Hoje"/"Ontem" e "dias sem vender" leem o **calendário local de quem opera** — é isso que o restaurante espera. Mas os testes foram escritos com datas fixas assumindo UTC-3, então passavam no notebook e quebravam na CI em UTC.

**Solução aplicada:** bloco `test.env` em `vitest.config.js` — `TZ: "America/Sao_Paulo"` fixa o fuso da suíte (determinística em qualquer máquina, sem tocar na lógica de produção) e credenciais falsas de Supabase (`http://supabase.teste.invalid`) satisfazem a checagem de importação. Nenhuma requisição real sai daí: o client nunca chega a ser usado, sempre é dublado. Resultado: **207 arquivos, 3612 testes, todos verdes**.

**Limite declarado:** o fuso fixo esconde bugs reais de fuso. Se um dia o produto atender tenant fora de `America/Sao_Paulo`, o certo é o teste construir as datas a partir do fuso do tenant em vez de o config fixar um.
...
**Fechamento da parte que ficou pendente (2026-08-15):** a multiplicação entre itens na falha sistêmica foi resolvida. `decidirAlertaDeLote(itens, limiar = 3)` (pura, exportada) classifica as baixas recusadas da **mesma operação** em `nenhum` / `individual` / `sistemica`; `iniciarLoteDeBaixas()` + `fecharLoteDeBaixas()` acumulam as falhas em vez de alertá-las uma a uma. De 3 falhas para cima sai um único insight com a chave **fixa** `estoque:baixa-falhou:sistemica`, o que faz o dedupe já existente colapsar uma queda de RLS inteira num alerta aberto só. O texto diz o que de fato está acontecendo — não é problema de um produto, é o sistema que não está descontando — em vez de mandar "confira a contagem deste item", conselho errado nesse caso. Aplicado nos dois lugares onde as baixas acontecem em série: `useFinalizarPagamento` (a venda) e `drenarPendenciasOffline` (o reenvio da fila), sempre em `try/finally`.

**Não coberto (segue pendente):** avisar o operador na própria tela do PDV — interromper a fila do caixa com erro de estoque é decisão de produto, ainda não escrita. `entradaEstoque` **não** é pendência: ao contrário do que esta linha dizia, ela emite evento, desfaz o otimista e devolve `{ error }`, que a `EstoqueView` mostra na tela.

**Referências:** `specs/td012-baixa-de-estoque-que-falha-em-silencio.md`, `specs/td012-alerta-agregado-falha-sistemica-de-baixa.md`; S2-1 em `sprint_pre_venda.md`.
