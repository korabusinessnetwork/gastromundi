...
| TD018 | Âmbar de status em `#f59e0b` literal **fora** do Console — mais de 30 ocorrências em `PDVView`, `CozinhaView`, `EstoqueView`, `NotasFiscaisTab`, `ImpostosAdmin`, `ImportarExportarTab`, `ConfiguracoesView`, `JarvasPanel`, `FechamentoModal`, `AssinaturaBanner`, `roles.js` e `crypto.js`, boa parte delas em `style=` inline no JSX | 🧹 Code Quality | Médio (cor que o tenant white-label não consegue trocar, decisão 017) | Médio (mexe em JSX, não só em CSS) | 🟡 Medium | Resolvido (2026-08-24, rodada final app-wide) — as duas constantes `AMBER` (`ComandaGrid.jsx`, `CozinhaView.jsx`) deixaram de existir e os usos em `PDVView/ComandaGrid`, `PDVView/MesaMapView`, `CozinhaView`, `EstoqueView`, `AdminView`, `ConfiguracoesView`, `JarvasPanel`, `FechamentoModal`, `AssinaturaBanner`, `roles.js`, `ImportarExportarTab.css` e `DemoClientes.css` viraram `varColor(C.warn)` / `alfa(C.warn, …)` / `var(--gm-warn)`, preservando os sufixos de alfa originais; `comprovante.css` usa `var(--gm-warn, #f59e0b)` porque a janela de impressão não herda o tema. **Três exceções documentadas no próprio código**, onde o âmbar é a n-ésima cor de uma paleta categórica e não a cor de alerta: `METODOS_COLOR` (Pix) em `PDVView/index.jsx`, `COR_TIPO` (COFINS) em `AdminView.jsx` e a escala de força de senha em `crypto.js` — tokenizar um membro faria só aquele chip seguir a cor de alerta do tenant. Comentários falsos de `colorAlfa.js` e `ConfiguracoesView.css` corrigidos |
