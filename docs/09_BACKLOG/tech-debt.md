...
| TD008 | Rate limiting de login só no cliente (contornável) | 🔒 Segurança | Baixo (Supabase Auth tem proteção própria) | Baixo | 🟢 Low | Resolvido (2026-08-10) — a contagem passou ao banco em `20260920_login_rate_limit_servidor.sql`: a RPC `registrar_tentativa_login(p_email, p_senha)` (SECURITY DEFINER, executável por `anon` porque roda antes de existir sessão) confere a senha com `crypt` contra `auth.users` e só então soma a falha, então o contador anda por erro REAL — limpar o `localStorage` ou chamar a API do Auth direto não escapa mais. Senha certa passa e zera o contador mesmo bloqueado, senão a trava viraria negação de serviço contra o próprio cliente (o username é público). O contador do navegador continua como primeira linha (resposta imediata e offline), e o cliente **falha aberto** de propósito (`src/lib/seguranca/travaLogin.js`): banco sem a migração, RPC fora do ar ou rede oscilando não podem trancar o restaurante fora do próprio PDV. Guards: `src/lib/seguranca/loginRateLimitSqlGuard.test.js` e `src/context/AppContext.travaLogin.test.jsx` |
