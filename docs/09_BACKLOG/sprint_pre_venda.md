...
| S1-3 | **Configurações do estabelecimento (admin do TENANT — o dono do restaurante)** — fim do "só por SQL", escopado ao próprio tenant: identidade/tema (logo, cores, nome), usuários, config de impressão/perfil, e visualização do próprio plano/assinatura. (A concessão de tempo de assinatura fica no console da plataforma, S1-2, não aqui.)<br>**Parcial (2026-08-01)** — a **visualização do próprio plano/assinatura** está entregue: aba "Minha assinatura" em Configurações (gerente/admin), com situação, plano, o que inclui e histórico de pagamentos, somente leitura (decisão 027). Falta: identidade/tema (logo esbarra na pendência de Storage × RLS), usuários e config de impressão/perfil. | Não dá pra entregar um sistema que o cliente só configura por SQL. Reaproveita telas já tokenizadas (F018). | 🟠 Médio |
