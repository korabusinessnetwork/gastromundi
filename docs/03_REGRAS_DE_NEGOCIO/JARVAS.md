...
|------|-----------|--------|
| 1 | Infraestrutura: tabelas `jarvas_eventos` + `jarvas_insights` (`supabase/migrations/20260703_jarvas.sql`) e serviço `src/lib/jarvas/jarvas.js` (emitirEvento fire-and-forget, registrarInsight, buscarInsights, atualizarStatusInsight). RLS: eventos por qualquer logado; insights estratégicos só gerente/admin | ✅ 2026-07-03 |
...
| 3 | Motor de regras (`src/lib/jarvas/jarvasEngine.js`): ruptura/estoque baixo, divergência de caixa, produto em alta/queda (7d vs 7d), cancelamentos recorrentes por operador. Dedupe por `origem.chave`, throttle 6h, roda pós-login de gerente/admin (fire-and-forget no `AppContext`) | ✅ 2026-07-03 |
| 4 | UI: `src/components/ui/paineis/JarvasPanel.jsx` — sino flutuante com badge de não lidos + painel lateral no `DesktopLayout`; severidade por cor do Design System, ação de um toque (navega e marca `executado`), marcar lido/descartar com auditoria; painel atualiza em tempo real (canal `jarvas-insights-realtime`, respeitando RLS) | ✅ 2026-07-03 |
| 5 | Previsões e assistente conversacional: (a) previsão de ruptura por consumo médio 14d e previsão de faturamento semanal por média móvel, no `jarvasEngine.js` (determinísticas, declaradas como estimativa); (b) Edge Function `jarvas-assistente` (agrega vendas 30d/estoque/fechamentos/insights no servidor e consulta a API Anthropic — chave só no Supabase, acesso só admin/gerente) + aba "Perguntar ao Jarvas" no `JarvasPanel`. Requer `supabase secrets set ANTHROPIC_API_KEY=...` e `supabase functions deploy jarvas-assistente --no-verify-jwt` | ✅ 2026-07-03 |
