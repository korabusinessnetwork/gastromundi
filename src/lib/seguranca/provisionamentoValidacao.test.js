...
const MODAL = join(RAIZ, "src", "components", "console", "modais", "NovoEstabelecimentoModal.jsx");
...
describe("a normalização da borda não divergiu da do Console", () => {
  // Não há como importar src/lib/console/console.js dentro da Edge Function (o deploy
...
describe("os limites da borda estão amarrados a uma fonte independente", () => {
  ...
  it("os mínimos são os mesmos que o Console já cobrava", () => {
    // `validarNovoEstabelecimento` (src/lib/console/console.js) é a validação da tela.
