...
describe("AnalyticsDashboard, base vazia e linguagem", () => {
  ...
  it("resposta atrasada de um período já trocado não pinta a tela", async () => {
    ...
    await act(async () => {
      de90.resolver({
        data: [{ tenant_id: "t-forte", faturamento_centavos: 990000, pedidos: 99, ultima_venda: diasAtras(0) }],
        error: null,
      });
    });
    await act(async () => {
      de30.resolver({
        data: [{ tenant_id: "t-forte", faturamento_centavos: 330000, pedidos: 33, ultima_venda: diasAtras(0) }],
        error: null,
      });
    });
