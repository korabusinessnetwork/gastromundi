...
vi.mock("@/lib/financeiro/financeiro", () => ({ criarLancamento: (...args) => criarLancamentoMock(...args) }));

const emitirDocumentoFiscalMock = vi.fn(() => Promise.resolve({ status: "stub", vendaId: "v1" }));
vi.mock("@/lib/fiscal/fiscal", () => ({ emitirDocumentoFiscal: (...args) => emitirDocumentoFiscalMock(...args) }));

const processarPagamentoTefMock = vi.fn(() => Promise.resolve({ status: "stub", metodo: "credito" }));
vi.mock("@/lib/vendas/tef", async () => {
  const actual = await vi.importActual("@/lib/vendas/tef");
