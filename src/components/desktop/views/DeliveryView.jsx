...
function AbaCardapio({
  ...
  // As duas frentes filtraram esta grade ao mesmo tempo, uma pela busca com
  // atalho de indisponíveis, a outra pela busca com categoria. É o mesmo
  // filtro, então aqui ele é um só, com as três peças: sem isso a tela
  // ficaria com dois campos de busca, um embaixo do outro.
  const [busca, setBusca] = useState("");
  ...
  // casa nome sem acento e sem caixa, que é o que faz "acai" achar "açaí".
  // Daqui só interessa QUAIS itens casam: a ordem da grade continua sendo a
  // `ordem` do cardápio, não a alfabética do helper.
  const itensFiltrados = useMemo(() => {
    ...
    return itens.filter(
      (it) =>
        ...
        (!soIndisponiveis || !it.disponivel)
    );
  }, [itens, busca, catFiltro, soIndisponiveis]);
  ...
  return (
    <>
      ...
      {/* Barra de filtro do cardápio publicado. A categoria só entra quando há
          mais de uma para escolher: num cardápio de uma categoria só, o campo
          é estorvo. O atalho de indisponíveis aparece sempre, porque saber
          quantos produtos estão fora do ar vale mesmo num cardápio pequeno. */}
